...
| `OCTO_SERVER_TOKEN` | service token for octo-server lookups on the human path (optional). The bot path (`/docs/v1/bot`) resolves the anti ghost-member existence check via the bot's own token against the bot user-info route, so this service token is no longer required for bot member-add or forward-grant | §4.7 / v4.3 bot API |
