...
// Runtime import via ESM. We cast through `unknown` so @types/jsdom does not pull lib:dom
// globally into this module's type scope (the DomDocument shim below is intentionally minimal).
import { JSDOM as JSDOMImpl } from 'jsdom'

const JSDOM = JSDOMImpl as unknown as {
  new (html: string): { window: { document: DomDocument } }
}

// Minimal local DOM types so we don't need lib: dom in tsconfig.
interface DomNode {
  nodeType: number
...
interface DomElement extends DomNode {
  tagName: string
  children: ArrayLike<DomElement> & Iterable<DomElement>
...
function parseHtmlDocument(html: string): DomDocument | null {
  try {
...
function domInlineToNodes(root: DomElement): PmNode[] {
  const out: PmNode[] = []
  const walk = (node: DomNode, marks: Array<{ type: string; attrs?: Record<string, unknown> }>) => {
    if (node.nodeType === 3) {
      if (node.textContent) out.push({ type: 'text', text: node.textContent, ...(marks.length ? { marks } : {}) })
      return
    }
    ...
    if (tag === 'strong' || tag === 'b') next = [...marks, { type: 'bold' }]
    ...
    else if (tag === 'sub') next = [...marks, { type: 'subscript' }]
    else if (tag === 'sup') next = [...marks, { type: 'superscript' }]
    else if (tag === 'mark') {
      const style = el.getAttribute('style') ?? ''
      const color = isSafeCssColor(/(?:^|;)\s*background-color\s*:\s*([^;]+)/i.exec(style)?.[1])
      next = [...marks, color ? { type: 'highlight', attrs: { color } } : { type: 'highlight' }]
    } else if (tag === 'span') {
      ...
      for (const declaration of (el.getAttribute('style') ?? '').split(';')) {
        ...
        if (property === 'color') attrs.color = isSafeCssColor(value) ?? undefined
        else if (property === 'font-size') attrs.fontSize = isSafeFontSize(value) ?? undefined
        else if (property === 'font-family') attrs.fontFamily = isSafeFontFamily(value) ?? undefined
      }
      for (const key of Object.keys(attrs)) if (attrs[key] == null) delete attrs[key]
      next = [...marks, Object.keys(attrs).length ? { type: 'textStyle', attrs } : { type: 'textStyle' }]
    }
    else if (tag === 'a') {
      const href = isSafeHref(el.getAttribute('href'))
      if (href) next = [...marks, { type: 'link', attrs: { href } }]
    }
...
export function isSafeFontFamily(raw: string | null | undefined): string | null {
  if (!raw) return null
  const families = raw.split(',').map((family) => family.trim()).filter(Boolean)
  if (families.length === 0 || families.length > 8) return null
  return families.every((family) =>
    /^(?:[A-Za-z\u3400-\u9fff][A-Za-z0-9 \u3400-\u9fff_-]*|"[A-Za-z\u3400-\u9fff][A-Za-z0-9 \u3400-\u9fff_-]*"|'[A-Za-z\u3400-\u9fff][A-Za-z0-9 \u3400-\u9fff_-]*')$/.test(family),
  ) ? families.join(', ') : null
}
...
export function parseInlineHtml(
  ...
  // Compatibility for Markdown produced before empty table cells stopped using
  // the document-level sentinel. Inside a GFM cell markdown-it tokenizes the
  // opening and closing tags as html_inline; consuming those exact tags leaves
  // an empty inline run, which mapTable correctly turns into an empty paragraph.
  // The standalone html_block form is handled by parseHtmlBlock below and still
  // creates an intentional top-level empty paragraph.
  if (/^<p\s+data-octo-empty-paragraph(?:="")?\s*>$/i.test(trimmed)) {
    markStack.push({ type: '__octoEmptyParagraph' })
    return { textNode: null }
  }
  if (/^<\/p>$/i.test(trimmed)
    && markStack[markStack.length - 1]?.type === '__octoEmptyParagraph') {
    markStack.pop()
    return { textNode: null }
  }
  ...
  if (/^<\/sub>$/i.test(trimmed)) {
    popMark(markStack, 'subscript')
    return { textNode: null }
  }
  if (/^<sup>$/i.test(trimmed)) {
    markStack.push({ type: 'superscript' })
    return { textNode: null }
  }
  if (/^<\/sup>$/i.test(trimmed)) {
    popMark(markStack, 'superscript')
    return { textNode: null }
  }
  ...
  if (spanOpen) {
    ...
    markStack.push(
      Object.keys(attrs).length > 0 ? { type: 'textStyle', attrs } : { type: 'textStyle' },
    )
    return { textNode: null }
  }
  if (/^<\/span>$/i.test(trimmed)) {
    popMark(markStack, 'textStyle')
    return { textNode: null }
  }
...
export function parseHtmlBlock(
  ...
  // Canonical Markdown exporter sentinel for a real empty PM paragraph. A
  // plain blank line is only a block separator in Markdown and cannot carry
  // this semantic by itself.
  if (/^<p\s+data-octo-empty-paragraph(?:="")?\s*><\/p>$/i.test(trimmed)) {
    return [{ type: 'paragraph', content: [] }]
  }

  const attachmentDoc = parseHtmlDocument(trimmed)
  const attachmentEl = attachmentDoc?.querySelector('div[data-file-attachment]') ?? null
  if (attachmentEl) {
    const src = attachmentEl.getAttribute('data-src') ?? ''
    const safeSrc = isSafeHref(src)
    if (!safeSrc) return []
    const rawSize = attachmentEl.getAttribute('data-size-bytes')
    const size = rawSize == null ? null : Number(rawSize)
    const attrs: Record<string, unknown> = {
      src: safeSrc,
      fileName: attachmentEl.getAttribute('data-file-name') || attachmentEl.textContent || 'attachment',
    }
    const mime = attachmentEl.getAttribute('data-mime')
    if (mime) attrs.mime = mime
    if (size != null && Number.isFinite(size) && size >= 0) attrs.sizeBytes = size
    return [{ type: 'fileAttachment', attrs }]
  }

  // Callout: <div data-callout data-variant="info">…</div>
  const calloutMatch =
    /^<div\s+data-callout\s+data-variant="([^"]*)">\s*([\s\S]*)\s*<\/div>$/i.exec(trimmed)
  if (calloutMatch) {
  ...
  const detailsMatch =
    /^<details>\s*<summary>([\s\S]*?)<\/summary>\s*([\s\S]*)\s*<\/details>$/i.exec(trimmed)
  if (detailsMatch) {
    ...
    return [
      {
        ...
        content: [
          {
            type: 'detailsSummary',
            content: summaryNodes.length ? summaryNodes : [],
          },
          {
            type: 'detailsContent',
            content: innerBlocks.length ? innerBlocks : [{ type: 'paragraph' }],
  ...
  if (/^<table[\s>]/i.test(trimmed)) {
    const table = parseHtmlTable(trimmed, inlineMap, warnings, blockMap)
...
/**
 * Parse an exporter HTML-fallback `<table>` into a PM table node. Uses jsdom on Node (DOMParser in
 * the browser) rather than regex, so it correctly handles NESTED tables and block cell content
 * (images, lists, paragraphs) that the exporter emits when a cell can't fit a single GFM pipe cell.
 * A cell's children are mapped recursively to PM blocks; a cell that is only inline content
 * collapses to a single paragraph (matching a plain cell).
 */
function parseHtmlTable(html: string, inlineMap: InlineMapper, warnings: string[], blockMap?: BlockMapper): PmNode | null {
  ...
  if (!root) return parseHtmlTableRegex(html, inlineMap, warnings)
  const table = domTableToNode(root, inlineMap, warnings, 0, blockMap)
...
const MAX_HTML_TABLE_DEPTH = 50

function domTableToNode(tableEl: DomElement, inlineMap: InlineMapper, warnings: string[], depth = 0, blockMap?: BlockMapper): PmNode {
  if (depth > MAX_HTML_TABLE_DEPTH) {
    warnings.push('docs.import.markdownNestingLimit')
    const text = (tableEl.textContent ?? '').trim()
    return { type: 'table', content: text ? [{ type: 'tableRow', content: [{ type: 'tableCell', content: [{ type: 'paragraph', content: inlineMap(text, warnings) }] }] }] : [] }
  }
  const rows: PmNode[] = []
  const trEls: DomElement[] = []
  for (const section of Array.from(tableEl.children)) {
    ...
    if (tag === 'tr') trEls.push(section)
      ...
      for (const tr of Array.from(section.children))
        if (tr.tagName.toLowerCase() === 'tr') trEls.push(tr)
    }
  }
  for (const tr of trEls) {
    ...
    for (const cellEl of Array.from(tr.children)) {
      ...
      const blocks = domCellToBlocks(cellEl, inlineMap, warnings, depth, blockMap)
      const cell: PmNode = {
...
function domCellToBlocks(
  ...
  for (const child of Array.from(cellEl.childNodes)) {
    ...
    if (tag === 'table') {
      flushInline()
      out.push(domTableToNode(el, inlineMap, warnings, depth + 1, blockMap))
    } else if (tag === 'img') {
      flushInline()
      ...
      flushInline()
      const inline = domInlineToNodes(el)
      ...
      flushInline()
      const inner = domCellToBlocks(el, inlineMap, warnings, depth + 1, blockMap)
      out.push({
        type: 'blockquote',
...
function domImgToNode(el: DomElement): PmNode | null {
...
function domListToNode(
  ...
  for (const liEl of Array.from(el.children)) {
    ...
    const blocks = domCellToBlocks(liEl, inlineMap, warnings, 0)
    items.push({
      type: 'listItem',
      content: blocks.length ? blocks : [{ type: 'paragraph' }],
    })
...
function parseHtmlTableRegex(
  ...
  while ((trMatch = trRe.exec(html))) {
    ...
    while ((cellMatch = cellRe.exec(trMatch[1] ?? ''))) {
      ...
      const cell: PmNode = {
        type: tag === 'th' ? 'tableHeader' : 'tableCell',
        content: cellInline.length
          ? [{ type: 'paragraph', content: cellInline }]
          : [{ type: 'paragraph' }],
      }
      if (colspan > 1 || rowspan > 1) {
        cell.attrs = {}
...
function parseAttr(attrStr: string, name: string): number {
  ...
  return m && m[1] != null ? Math.max(1, parseInt(m[1], 10)) : 1
}
