...
function listLineOf(
  ...
  if (numPr) {
    ...
    if (numId != null) {
      ...
      // Clamp the level to the OOXML-legal range (w:ilvl is 0..8). This is
      // lossless for valid documents and prevents a crafted `<w:ilvl
      // w:val="10000000"/>` from making buildList open millions of nested
      // frames (an output-side resource-exhaustion DoS the input byte caps do
      // not bound). Mirrors the task path's Math.min(depth, 8).
