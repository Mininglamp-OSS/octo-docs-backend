...
/**
 * A structural time-budget guard. Matches the `Deadline` interface in
 * document.ts (kept structural here to avoid a circular import). `check()`
 * throws once the budget is exceeded. Optional: when omitted, no checks run.
 */
interface Deadline {
  check(): void
}

/** Tick the deadline every N iterations to bound the row/cell walk without
 *  paying a Date.now() on every single cell. */
const DEADLINE_TICK = 256
...
export function mapTable(
  ...
  // Bound the row/cell walk. mapCellBlocks already checks the deadline for a
  // cell's CONTENT, but empty cells and vMerge="continue" cells skip that
  // callback entirely, so a table of many empty/continuation cells would spin
  // these loops uninterrupted. Tick every DEADLINE_TICK iterations here too.
  let ticks = 0
  const tick = (): void => {
    if (deadline && ++ticks % DEADLINE_TICK === 0) deadline.check()
  }
