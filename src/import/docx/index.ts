...
import { walkDocument } from './document.js'
...
export function buildDocFromParts(extracted: ExtractedDocx): DocxImportResult {
  ...
  const walked = walkDocument(documentXml, rels.targets, numbering)
...
export async function importDocx(buffer: Buffer): Promise<DocxImportResult> {
  ...
  return buildDocFromParts(extracted)
}
