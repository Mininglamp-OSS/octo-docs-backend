...
function renderImage(node: PMNode, ctx: RenderCtx): string {
  ...
  if (attachId && ctx.attachments.has(attachId)) {
    const resolved = ctx.attachments.get(attachId)!
    src = resolved.url
    mime = resolved.mime
  }
  ...
  if (!src) return ''
  // Only render image MIME types — a crafted image node could point at a
  // same-document non-image attachment and make Chrome fetch a larger file.
...
function renderBookmark(node: PMNode): string {
  ...
  return (
    ...
    (url ? `<div class="bookmark-url">${siteName ? siteName + ' · ' : ''}${escText(url)}</div>` : '') +
    `</div>`
  )
}
