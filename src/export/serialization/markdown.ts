...
import { assertPmTreeBounds, sanitizeLinkHref, sanitizeBookmarkUrl, safeCssColor, safeFontSize, explicitFontFamily, type PmNode } from './policy.js'

export interface ResolvedAttachment { attachId: string; url: string; fileName: string; mime?: string; sizeBytes?: number }
const INDENT_STEP_EM = 2
function clampIndent(value: unknown): number { const n = Number(value); return Number.isFinite(n) ? Math.max(0, Math.min(8, Math.round(n))) : 0 }
...
export interface ExportOptions {
  ...
  /** Optional localized signed-link notice. Omit for canonical/golden output. */
  signedLinkNotice?: string
...
function exportHeader(notice?: string): string { return notice ? `<!-- ${notice} -->` : '' }
...
export async function exportDocToMarkdown(
  ...
  const header = exportHeader(opts.signedLinkNotice)
  return header ? (body ? `${header}\n\n${body}\n` : `${header}\n`) : (body ? `${body}\n` : '')
}

/** Pure synchronous canonical serializer for runtimes that pre-hydrate attachment URLs. */
export function serializeMarkdown(doc: MdNode, opts: Pick<ExportOptions, 'emojiGlyph' | 'signedLinkNotice'> = {}): string {
  assertPmTreeBounds(doc)
  const body = serializeBlocks(doc.content ?? [], { urls: new Map(), emojiGlyph: opts.emojiGlyph })
  const header = exportHeader(opts.signedLinkNotice)
  return header ? (body ? `${header}\n\n${body}\n` : `${header}\n`) : (body ? `${body}\n` : '')
}
...
function serializeBlock(node: MdNode, ctx: Ctx): string {
  switch (node.type) {
    case 'paragraph':
      // Markdown blank lines only separate blocks; they do not represent an
      // actual empty paragraph. Use a narrow, importer-owned HTML sentinel so
      // intentional spacer paragraphs survive export/re-import instead of
      // disappearing (notably around formula sections and after long tables).
      return (node.content?.length ?? 0) === 0
        ? '<p data-octo-empty-paragraph></p>'
        : wrapAlign(escapeLeadingBlockMarkers(serializeInline(node.content ?? [], ctx)), node)
    case 'heading': {
    ...
    case 'codeBlock': {
      const lang = typeof node.attrs?.language === 'string' ? node.attrs.language.replace(/[`\r\n]/g, '') : ''
...
function serializeListItem(
  ...
  // A GFM task checkbox is parsed as content inside a normal `- ` list item;
  // its full textual prefix (`- [x] ` = 6 chars) is NOT the CommonMark marker
  // width. Indenting a nested task by all 6 spaces turns it into lazy/code
  // continuation in markdown-it and flattens `parent -> child` into one line.
  // Four spaces is accepted by CommonMark/GFM and preserves the nested list.
  const childIndent = indent + ' '.repeat(item.type === 'taskItem' ? 4 : marker.length)
...
function cellText(cell: MdNode, ctx: Ctx): string {
  ...
  // The document-level empty-paragraph sentinel must never become cell text:
  // GFM already represents an empty cell losslessly, while parsing the raw HTML
  // sentinel inline would turn it into visible literal text. Multi-block cells
  // use the HTML-table path below, where real <p></p> elements preserve spacer
  // paragraphs without involving the Markdown sentinel.
  const blocks = cell.content ?? []
  if (blocks.length === 1 && blocks[0]?.type === 'paragraph' && (blocks[0].content?.length ?? 0) === 0) {
    return ''
  }
...
function serializeTable(node: MdNode, ctx: Ctx): string {
  ...
  const grid = rows.map((row) =>
    // Backslashes inside math are TeX syntax and must remain byte-for-byte.
    // Encode the GFM cell delimiter as an HTML entity instead of introducing
    // a Markdown backslash escape that static analysis can mistake for partial
    // string escaping (and that can interact with adjacent TeX escapes).
...
function blockToHtml(node: MdNode, ctx: Ctx): string {
  switch (node.type) {
    ...
    case 'image': {
      const src = sanitizeLinkHref(imageSrc(node, ctx))
...
function serializeImage(node: MdNode, ctx: Ctx): string {
  const url = safeHref(imageSrc(node, ctx), sanitizeLinkHref)
...
function serializeFileAttachment(node: MdNode, ctx: Ctx): string {
  ...
  // The server-side export path pre-hydrates attrs.src, while the browser path
  // supplies the same URL through ctx.urls. Preserve the atom as allowlisted
  // HTML instead of degrading it to an ordinary link, so re-import can recover
  // the fileAttachment node and its durable source marker.
  const rawSrc = resolved?.url ?? (typeof node.attrs?.src === 'string' ? node.attrs.src : '')
  const src = safeHref(rawSrc, sanitizeLinkHref)
  if (src) {
    const attrs = [
      'data-file-attachment',
      `data-src="${escapeHtmlAttr(src)}"`,
      `data-file-name="${escapeHtmlAttr((typeof node.attrs?.fileName === 'string' && node.attrs.fileName) || resolved?.fileName || 'attachment')}"`,
    ]
    const mime = (typeof node.attrs?.mime === 'string' && node.attrs.mime) || resolved?.mime
    const size = typeof node.attrs?.sizeBytes === 'number' ? node.attrs.sizeBytes : resolved?.sizeBytes
    if (mime) attrs.push(`data-mime="${escapeHtmlAttr(mime)}"`)
    if (typeof size === 'number' && Number.isFinite(size) && size >= 0) attrs.push(`data-size-bytes="${size}"`)
    return `<div ${attrs.join(' ')}>${escapeHtmlText((typeof node.attrs?.fileName === 'string' && node.attrs.fileName) || resolved?.fileName || 'attachment')}</div>`
  }
...
function serializeInlineNode(node: MdNode, ctx: Ctx): string {
  switch (node.type) {
    case 'text':
      // A literal string such as "&amp;" is otherwise decoded by Markdown's
      // HTML-entity pass into "&" on re-import. Escape the ampersand of only
      // the five named XML/HTML entities so visible entity text remains text;
      // ordinary ampersands and Markdown syntax stay readable and unchanged.
      return applyMarks(escapeLiteralNamedEntities(node.text ?? ''), node.marks ?? [])
    case 'hardBreak':
      return '  \n'
    case 'image':
      return serializeImage(node, ctx)
...
function escapeLiteralNamedEntities(text: string): string {
  return text.replace(/&(amp|lt|gt|quot|apos);/gi, '&amp;$1;')
}
  ...
  for (const mark of marks) {
    switch (mark.type) {
      ...
      case 'highlight': {
        ...
        out =
          safeCssColor(hl)
            ? `<mark style="background-color:${escapeHtmlAttr(safeCssColor(hl)!)}">${out}</mark>`
            : `<mark>${out}</mark>`
        break
      }
      case 'textStyle': {
        ...
        const fontFamily = explicitFontFamily(mark.attrs?.fontFamily)
        ...
        const decls: string[] = []
        if (color) decls.push(`color:${escapeHtmlAttr(color)}`)
        if (fontSize) decls.push(`font-size:${escapeHtmlAttr(fontSize)}`)
        if (fontFamily) decls.push(`font-family:${escapeHtmlAttr(fontFamily)}`)
