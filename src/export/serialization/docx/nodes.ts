...
import { convertInlineContent, iconPrefix } from './marks.js'
import { isSafeHref } from './href-safety.js'
import { latexToMathComponent } from './math.js'

/** Re-exported so existing importers (e.g. href-safety.test.ts) keep working. */
export { isSafeHref } from './href-safety.js'
...
import { getImageAsset, getImageDimensions } from './images.js'
import { FONT_CODE, mapTextAlign, mapSpacing } from './styles.js'
import type { MdNode, DocxContext } from './types.js'
...
function convertHeading(node: MdNode, ctx: DocxContext): Paragraph {
  const runs = convertInlineContent(node.content ?? [], ctx.emojiGlyph, true)
  const align = mapTextAlign(node.attrs?.textAlign)
  const spacing = mapSpacing(node.attrs)
  return new Paragraph({
    children: runs,
...
export function convertImage(node: MdNode, ctx: DocxContext): FileChild[] {
  ...
  // docx requires a raster fallback for SVG. Never label XML bytes as PNG:
  // Word treats that as a corrupt image and some consumers trust the declared
  // type when decoding. If fallback generation failed, use the ordinary
  // unavailable placeholder instead.
  if (asset?.type === 'svg' && !asset.fallback) {
    return [new Paragraph({ children: [new TextRun({ text: alt ? `[Image: ${alt}]` : '[Image unavailable]', italics: true, color: '888888' })] })]
  }
  ...
  let imgType: 'jpg' | 'png' | 'gif' | 'bmp' | 'svg' = asset?.type ?? 'png'
  ...
  return [
    new Paragraph({
      children: [
        new ImageRun(imgType === 'svg' && asset?.fallback
          ? {
              data: buffer,
              transformation: { width: dims.width, height: dims.height },
              type: 'svg',
              fallback: { data: asset.fallback, type: 'png' },
            }
          : {
              data: buffer,
              transformation: { width: dims.width, height: dims.height },
...
function convertFileAttachment(node: MdNode, ctx: DocxContext): Paragraph {
  const attachId = typeof node.attrs?.attachId === 'string' ? node.attrs.attachId : undefined
  const resolved = attachId ? ctx.urls.get(attachId) : undefined
  const sourceUrl = typeof node.attrs?.src === 'string' ? node.attrs.src : undefined
  const downloadUrl = resolved?.url ?? sourceUrl
  const name =
...
const CALLOUT_FILL: Record<string, string> = { info: 'E8F1FD', warn: 'FFF4E5', warning: 'FFF4E5', tip: 'D4EDDA', success: 'D4EDDA' }
    ...
    if (child.type === 'paragraph') {
      ...
      result.push(
        new Paragraph({
          ...
          style,
          shading: { fill: CALLOUT_FILL[variant] ?? 'E8F1FD' },
          indent: { left: 360 },
        }),
      )
    } else {
