...
/** Like `num` but clamps to a minimum value — for env vars where zero/negative breaks the feature. */
...
export const config = {
  ...
  pdfExport: {
    ...
    maxConcurrent: numMin('PDF_EXPORT_MAX_CONCURRENT', 2, 1),
    // Max renders allowed to WAIT in the queue; over this the route returns 503.
    maxQueue: numMin('PDF_EXPORT_MAX_QUEUE', 10, 1),
    // Hard per-render timeout; on expiry the page is closed (leak guard).
    renderTimeoutMs: numMin('PDF_EXPORT_RENDER_TIMEOUT_MS', 30_000, 1_000),
