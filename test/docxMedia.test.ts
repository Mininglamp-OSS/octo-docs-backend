...
import { sniffImageMime, resolveImages, imagePlaceholder, type MediaUploadCtx } from '../src/import/docx/media.js'
...
describe('resolveImages — degradation (never sinks the import)', () => {
  async function degradeReason(resolve: any, c = ctx()): Promise<string[]> {
    const doc: PmNode = { type: 'doc', content: [imagePlaceholder('rId1', null)] }
    const warnings: string[] = []
...
describe('walkDocument — drawing detection', () => {
  it('emits an image placeholder from a w:drawing a:blip r:embed', () => {
    ...
    expect((img.attrs as any)._embedRel).toBe('rId7')
    expect((img.attrs as any).alt).toBe('my chart')
  })
})
