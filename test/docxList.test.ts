...
describe('buildList — flat & nested', () => {
  ...
  it('nests a child list under the last item (ilvl 0 → 1 → 0)', () => {
    ...
    const nested = (items[0] as PmNode).content[1]
    expect(nested.type).toBe('bulletList')
    expect(itemText(nested.content[0])).toBe('a.1')
    expect(itemText(items[1] as PmNode)).toBe('b')
  })
})

describe('buildList — numId boundary', () => {
  const oline = (ilvl: number, text: string, numId: string, start?: number): ListLine => ({
    ilvl,
    kind: 'ordered',
    numId,
    start,
  ...
  it('splits two adjacent ordered lists with different numIds at the same level', () => {
    // numId=1 ("1,2") then numId=2 that must RESTART ("1,2") — they must not
    // merge into one "1,2,3,4" list.
    const roots = buildList([
      oline(0, 'a1', '1'),
      oline(0, 'a2', '1'),
      oline(0, 'b1', '2'),
      oline(0, 'b2', '2'),
    ...
    expect(roots[0]!.type).toBe('orderedList')
    expect(roots[0]!.content!.map((i) => itemText(i as never))).toEqual(['a1', 'a2'])
    expect(roots[1]!.type).toBe('orderedList')
    expect(roots[1]!.content!.map((i) => itemText(i as never))).toEqual(['b1', 'b2'])
  })

  it('keeps a single list when the numId is unchanged', () => {
    const roots = buildList([oline(0, 'a', '1'), oline(0, 'b', '1'), oline(0, 'c', '1')])
    expect(roots).toHaveLength(1)
    expect(roots[0]!.content!.map((i) => itemText(i as never))).toEqual(['a', 'b', 'c'])
  })

  it('preserves each list’s own start when a numId change restarts numbering', () => {
    const roots = buildList([oline(0, 'a', '1'), oline(0, 'b', '2', 5)])
    expect(roots).toHaveLength(2)
    // second list keeps its own w:start=5, not a continuation of the first
    expect(roots[1]!.attrs?.start).toBe(5)
  })
})
...
describe('buildList — mixed ordered/bullet', () => {
  it('emits two sibling top-level lists when kind changes at the same level', () => {
    ...
    expect(itemText(roots[0]!.content![0] as PmNode)).toBe('a')
    expect(roots[1]!.type).toBe('orderedList')
    expect(itemText(roots[1]!.content![0] as PmNode)).toBe('1')
  })
  ...
  it('nested list can be a different kind than its parent', () => {
    ...
    const nested = (list.content![0] as PmNode).content[1]
...
describe('buildList — ilvl jumps', () => {
  it('synthesises intermediate levels on a 0 → 2 jump', () => {
    ...
    const l1 = (list.content![0] as PmNode).content[1]
...
describe('buildList — task lists', () => {
  it('builds a taskList with checked state', () => {
    ...
    expect(list.content!.map((i: PmNode) => i.attrs.checked)).toEqual([false, true])
...
describe('parseNumbering', () => {
  ...
  it('does not hang on a non-finite or absurd w:ilvl (kindOf DoS guard)', () => {
    const n = parseNumbering(xml)
    // A crafted `<w:ilvl w:val="1e999"/>` parses to Infinity; the descending
    // fallback loop must not spin forever (Infinity - 1 === Infinity). A huge
    // finite level must resolve without a ~1e11-iteration burn. Each of these
    // returns synchronously; a regression here would time the test out.
    expect(n.kindOf('20', Infinity)).toBe('ordered')
    expect(n.kindOf('20', Number.MAX_SAFE_INTEGER)).toBe('ordered')
    expect(n.kindOf('20', NaN)).toBe('ordered')
    expect(n.kindOf('10', -5)).toBe('bullet')
  })
...
describe('walkDocument — list integration', () => {
  ...
  it('treats w:numId="0" (remove-numbering sentinel) as a plain paragraph, not a list', () => {
    // OOXML w:numId="0" explicitly cancels inherited numbering: the paragraph
    // must import as a plain paragraph, never a bullet/ordered list item.
    const inner =
      '<w:p><w:pPr><w:numPr><w:ilvl w:val="0"/><w:numId w:val="0"/></w:numPr></w:pPr><w:r><w:t>plain</w:t></w:r></w:p>'
    const out = walkDocument(doc(inner), new Map(), numbering)
    expect(out.content[0]).toEqual({ type: 'paragraph', content: [{ type: 'text', text: 'plain' }] })
    expect(out.content.some((n) => n.type === 'bulletList' || n.type === 'orderedList')).toBe(false)
  })
  ...
  it('detects a task item from a w14:checkbox content control', () => {
    ...
    expect((out.content[0]!.content![0] as PmNode).attrs.checked).toBe(true)
  })

  it('keeps a numbered paragraph that holds a display formula (m:oMathPara) as a list item', () => {
    // Regression ("12345 都没了"): a numbered list item whose only content is a
    // DISPLAY formula (m:oMathPara wrapper) was dropped — collectInline only
    // handled inline m:oMath, so the wrapped formula matched nothing and the
    // item became an empty paragraph with no warning. It must stay an
    // orderedList item with the math preserved (as inline math).
    ...
    // The display wrapper — this is the exact variant that was silently dropped.
    const mathPara =
      '<m:oMathPara><m:oMath><m:sSup><m:e><m:r><m:t>x</m:t></m:r></m:e><m:sup><m:r><m:t>2</m:t></m:r></m:sup></m:sSup></m:oMath></m:oMathPara>'
    const inner =
      `<w:p><w:pPr><w:pStyle w:val="ListParagraph"/><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr></w:pPr>${mathPara}</w:p>` +
      `<w:p><w:pPr><w:pStyle w:val="ListParagraph"/><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr></w:pPr>${mathPara}</w:p>`
    ...
    expect(out.content[0]!.type).toBe('orderedList')
    expect(out.content[0]!.content).toHaveLength(2)
    const para = (out.content[0]!.content![0] as PmNode).content[0]
    expect(para.type).toBe('paragraph')
    // The display formula survives as inline math, not an empty paragraph.
    expect(para.content).toHaveLength(1)
    expect(para.content[0].type).toBe('inlineMath')
    // No standalone blockMath leaked out of the list, and nothing was dropped.
    expect(out.content.some((n: PmNode) => n.type === 'blockMath')).toBe(false)
    expect(warnings).toHaveLength(0)
  })

  it('keeps a numbered paragraph that holds an inline formula (m:oMath) as a list item', () => {
    // Companion to the display-wrapper case: a bare inline m:oMath in a numbered
    // item must also stay an orderedList item with the math inline.
    const mdoc = (inner: string): Buffer =>
      Buffer.from(
        `<?xml version="1.0"?><w:document xmlns:w="http://x" xmlns:w14="http://14" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><w:body>${inner}</w:body></w:document>`,
        'utf8',
  ...
  it('clamps an out-of-range w:ilvl so a crafted deep level cannot blow up node count', () => {
    // Security: `<w:ilvl w:val="10000000"/>` must not open ~1e7 nested frames.
    // OOXML bounds w:ilvl to 0..8; the clamp keeps the tree shallow and bounded.
    const inner =
      '<w:p><w:pPr><w:numPr><w:ilvl w:val="10000000"/><w:numId w:val="1"/></w:numPr></w:pPr><w:r><w:t>deep</w:t></w:r></w:p>'
    const out = walkDocument(doc(inner), new Map(), numbering)
    // Measure the maximum list nesting depth of the produced tree.
    const maxDepth = (node: PmNode, d = 0): number => {
      if (!node || typeof node !== 'object') return d
      const here = node.type === 'orderedList' || node.type === 'bulletList' ? d + 1 : d
      let m = here
      for (const c of node.content ?? []) m = Math.max(m, maxDepth(c, here))
      return m
    }
    const depth = Math.max(...out.content.map((n: PmNode) => maxDepth(n)))
    expect(depth).toBeLessThanOrEqual(9) // 8 levels + 1 for the counting offset
  })

  it('drops a table nested past the max block depth instead of overflowing the stack', () => {
    // Security: a deeply nested <w:tbl> must degrade gracefully (drop + warn)
    // rather than recurse without bound. 30 levels (90 tags) stays under the
    // XML parser's nested-tag ceiling but exceeds MAX_BLOCK_DEPTH (20).
    const openCell = '<w:tbl><w:tr><w:tc>'
    const closeCell = '</w:tc></w:tr></w:tbl>'
    const N = 30
    const inner = openCell.repeat(N) + '<w:p><w:r><w:t>x</w:t></w:r></w:p>' + closeCell.repeat(N)
    const out = walkDocument(doc(inner), new Map(), numbering)
    expect(Array.isArray(out.content)).toBe(true)
    expect(out.warnings.some((w: string) => /nested table/i.test(w))).toBe(true)
  })
})
