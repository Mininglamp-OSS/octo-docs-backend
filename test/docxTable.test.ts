...
describe('mapTable — horizontal merge (gridSpan)', () => {
  it('maps gridSpan to colspan', () => {
    ...
    expect((wide.attrs as Record<string, unknown>).colspan).toBe(2)
...
describe('mapTable — vertical merge (vMerge)', () => {
  it('collapses a vMerge restart+continue into a single rowspan=2 cell', () => {
    ...
    expect((merged.attrs as Record<string, unknown>).rowspan).toBe(2)
  ...
  it('treats a bare w:vMerge (no val) as continue', () => {
    ...
    const t = table(walkDocument(xml, new Map()))
    expect((t.content![0]!.content![0]!.attrs as Record<string, unknown>).rowspan).toBe(2)
  ...
  it('spans three rows (restart + two continues = rowspan 3)', () => {
    ...
    const t = table(walkDocument(xml, new Map()))
    expect((t.content![0]!.content![0]!.attrs as Record<string, unknown>).rowspan).toBe(3)
  })
})
...
describe('mapTable — column widths', () => {
  it('reads tblGrid widths into colwidth (twips → px)', () => {
    ...
    const t = table(walkDocument(xml, new Map()))
    expect((t.content![0]!.content![0]!.attrs as Record<string, unknown>).colwidth).toEqual([100]) // 1500/15
    expect((t.content![0]!.content![1]!.attrs as Record<string, unknown>).colwidth).toEqual([200]) // 3000/15
  })
  ...
  it('sums covered columns for a colspan cell', () => {
    ...
    const t = table(walkDocument(xml, new Map()))
    expect((t.content![0]!.content![0]!.attrs as Record<string, unknown>).colwidth).toEqual([100, 100])
  })
})
