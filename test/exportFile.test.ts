...
beforeEach(() => {
  ...
  vi.mocked(docAttachmentRepo.listByDoc).mockResolvedValue([])
})
...
describe('GET /:docId/export/file handler', () => {
  ...
  it('hardens exported attachment links with trusted MIME and a sanitized download filename', async () => {
    vi.mocked(yDocStateToProsemirrorJSON).mockReturnValue({
      type: 'doc',
      content: [{ type: 'fileAttachment', attrs: { attachId: 'att_1' } }],
    })
    vi.mocked(docAttachmentRepo.listByDoc).mockResolvedValue([{
      attachId: 'att_1',
      docId: 'd1',
      objectKey: 'd1/att_1/payload.txt',
      mime: 'text/plain',
      sizeBytes: 12,
      fileName: 'bad"\r\nname.txt',
      createdBy: 'u1',
    ...
    const res = response()
    await exportFileHandler(req('md') as never, res as never)
    const markdown = (res.body as Buffer).toString('utf8').replace(/&amp;/g, '&')
    const src = /data-src="([^"]+)"/.exec(markdown)?.[1]
    expect(src).toBeTruthy()
    const url = new URL(src!)
    expect(url.searchParams.get('response-content-type')).toBe('text/plain')
    expect(url.searchParams.get('response-content-disposition')).toBe('attachment; filename="badname.txt"')
    expect(url.hash).toBe('#octo-attachment%3Ad1%3Aatt_1')
  })
