...
当前 `SCHEMA_VERSION = 17`（`src/schema/index.ts`）—— v5–v15 已与前端 `@octo/docs-schema` **原子同落**；v16 为「正文字体」特性的前后端共享契约（后端 `buildSchema()` 在 `textStyle` mark 上新增 `fontFamily` attr），已合入 main。v17 rebase 到含 v16 的最新 main 后，`buildSchema()` 累积携带 v16 `fontFamily` + v17 block-spacing attrs（单调递增、砍号留 gap、不回收），v17 为 v16 的真超集。
